...
export default function LipsyncLibraryManager({ onClose, onApplyPreset }: {
  onClose: () => void;
  ...
  const fetchPresets = async () => {
    try {
      ...
      const response = await fetch(`${config.supabase_server_url}/lipsync_presets?user_id=eq.${LIBRARY_USER_ID}`, {
        headers: {
            ...
            <div className="flex items-center justify-center h-64">
              <div className="text-center">
                ...
                <p className="text-sm text-gray-500 dark:text-gray-500 mt-1">
                  No lipsync presets available in the library
                </p>
              </div>
            </div>
          ) : (
