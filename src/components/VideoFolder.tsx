...
import { Star, Search, Download, Share, Trash2, Filter, Calendar, Video, SortAsc, SortDesc, ZoomIn, Folder, Plus, ChevronRight, Home, ArrowLeft, Pencil, Menu, X, File, User, RefreshCcw, Edit, Play, QrCode } from 'lucide-react';
...
export default function VideoFolder({ onBack }: VideoFolderProps) {
  ...
  useEffect(() => {
    if (shareModal.open && shareModal.itemId && shareModal.itemPath) {
      // Find the video data for the given itemId
      const video = videos.find(v => v.video_id === shareModal.itemId);
      if (video) {
        const videoUrl = getVideoUrl(video);
        generateQRCode(videoUrl);
      }
    }
  }, [shareModal.open, shareModal.itemId, shareModal.itemPath, userData.id, videos]);
  ...
  const handleShare = (video: VideoData) => {
    const videoUrl = getVideoUrl(video);
    const videoId = video.video_id;
    const videoPath = video.video_path || 'video';
    setShareModal({ open: true, itemId: videoId, itemPath: videoPath });
  };
      ...
      {/* Pagination Controls */}
        ...
        <div className="flex flex-col sm:flex-row justify-between items-center gap-4 py-6 border-t border-gray-200 dark:border-gray-700 mt-4">
          ...
          <div className="text-sm text-gray-600 dark:text-gray-400">
            Showing {((currentPage - 1) * itemsPerPage) + 1}-{Math.min(currentPage * itemsPerPage, totalVideosCount)} of {totalVideosCount} videos (16 per page)
          </div>
                ...
                <div className="space-y-2">
                  ...
                  <div className="flex gap-2">
                    <Input
                      value={(() => {
                        const video = videos.find(v => v.video_id === shareModal.itemId);
                        return video ? getVideoUrl(video) : '';
                      })()}
                      readOnly
                      className="text-xs"
                    />
                    <Button
                      ...
                      onClick={() => {
                        const video = videos.find(v => v.video_id === shareModal.itemId);
                        if (video) {
                          copyToClipboard(getVideoUrl(video));
                        }
                      }}
                    >
                      Copy
  ...
  const shareToSocialMediaFromModal = (platform: string, itemId: string) => {
    // Find the video data for the given itemId
    const video = videos.find(v => v.video_id === itemId);
    if (!video) {
    ...
    switch (platform) {
      ...
      case 'linkedin':
        ...
        break;
      case 'pinterest':
        shareUrl = `https://pinterest.com/pin/create/button/?url=${encodeURIComponent(videoUrl)}&description=${encodeURIComponent(text)}`;
        break;
      default:
        return;
    }
